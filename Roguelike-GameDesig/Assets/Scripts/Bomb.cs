...
public class Bomb : MonoBehaviour
{
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        else if (collision.gameObject.CompareTag("Wind"))
        {
            ...
            wb2.AddForce(transform.right * 11, ForceMode2D.Impulse);
            wb2.AddForce(transform.up * 0.5f,ForceMode2D.Impulse);
            Rigidbody2D wb4 = wind4.GetComponent<Rigidbody2D>();
            wb4.AddForce(transform.right * 11, ForceMode2D.Impulse);
            wb4.AddForce(transform.up * -0.5f, ForceMode2D.Impulse);
